...
[
	{
		...
		if(!(_logic getVariable["digi_gas_active",false]) || !(alive _unit) || isNull _unit) then {
...
while{_logic getVariable ["digi_gas_active", false] && !(isNull _logic) && alive _unit && !(_unit getVariable ["ACE_isUnconscious",false])} do { 
	_pos = _logic getVariable ["digi_pos",[0,0,0]];
	if((_unit distance _pos) <= _radius_max && !(_unit getVariable["digi_medical_enteredPoisen",false])) then {
		_unit setVariable["digi_medical_enteredPoisen",true,true];
		private _fnc_afterwait = {
			params["_unit","_gastype","_pos","_skill"];
			if !(goggles _unit in DIGI_AVAIL_GASMASK) exitWith {
				if(_gastype == "CS") then {
					while{_unit distance _pos < 10 && _unit getVariable["digi_medical_enteredPoisen",false]} do {
						_unit say3D "cough_1";
					...
					if(ace_medical_statemachine_AIUnconsciousness) then {
						for "_i" from 0 to 10 step 1 do {
							[_unit, "RightArm", "PoisenBP"] call digi_fnc_medicationLocal;
							uiSleep 5;
						};
					} else {
