...
[{
	...
	if(_unit getVariable["digi_medical_enteredPoisen",false]) then {
		...
		_unit setVariable ["digi_medical_CS",false,true];
...
[{
	...
	private _pos = _logic getVariable ["digi_pos",[0,0,0]];
	if ( _unit distance _pos > _radius || !(_logic getVariable["digi_gas_active",false]) || isNull _logic ) exitWith {
